...
const productSchema = new mongoose.Schema({
  ...
  value: { 
    type: String, 
    enum: ["nrp", "mrp", "manual"],  // 🔥 Added "manual" option
    required: true 
  },
  ...
  items: [{
    ...
    manualPrice: {  // 🔥 NEW: Manual price field
      type: Number,
      default: 0
    }
  }],
  createdBy: {  // 🔥 Track which user created this
    type: mongoose.Schema.Types.ObjectId,
    ref: "User",
    required: false
  },
  createdByUsername: {  // 🔥 Store username for easy reference
    type: String,
    required: false
  }
}, {
    ...
    if (this.items && this.items.length > 0) {
      ...
      this.items.forEach(entry => {
        ...
        if (itemData) {
          ...
          // 🔥 NEW: Handle manual pricing
          let rate = 0;
          if (this.value === 'manual') {
            rate = entry.manualPrice || 0;
          } else if (this.value === 'nrp') {
            rate = itemData.nrp || 0;
          } else if (this.value === 'mrp') {
            rate = itemData.mrp || 0;
          }
      ...
      update.items.forEach(entry => {
        ...
        if (itemData) {
          ...
          if (update.value === 'manual') {
            rate = entry.manualPrice || 0;
          } else if (update.value === 'nrp') {
            rate = itemData.nrp || 0;
          } else if (update.value === 'mrp') {
            rate = itemData.mrp || 0;
          }
